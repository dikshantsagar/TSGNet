...
from datamodules.abberated_datamodule import AbberatedDataModule
...
def main(learning_rate, weight_decay, batch_size, gpus, max_epochs, downsample_rate, name):
    ...
    update_config(wandb_logger, {   "Dataset": "Abberated",
                                    'lr' : learning_rate,
    ...
    model_checkpoint = ModelCheckpoint(dirpath=f'../runs/abberated/{name}/', monitor='Loss/test')
    ...
    dm = AbberatedDataModule(directory="/baldig/physicsprojects/electron_microscopy", downsample_rate=downsample_rate, batch_size=batch_size)
